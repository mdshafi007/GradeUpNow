...
router.post('/profile', verifyFirebaseToken, [
  ...
  body('department').trim().isLength({ min: 2 }).withMessage('Department is required and must be at least 2 characters'),
  ...
  try {
    ...
    const { fullName, collegeName, department, role = 'admin' } = req.body;
...
router.get('/students', verifyFirebaseToken, verifyAdminRole, async (req, res) => {
  try {
    ...
    // Build query to find college students from admin's college and department only
    let query = {
      collegeCode: collegeCode, // Use collegeCode instead of collegeName
      department: req.admin.department, // Filter by admin's department
    ...
    const students = await CollegeStudent.find(query)
      .select('rollNumber name email department year semester batch section coursesEnrolled quizzesCompleted assignmentsDue overallGrade lastLoginAt createdAt')
      .sort({ createdAt: -1 })
      .skip(skip)
      .limit(limitNum);
    ...
    res.json({
      ...
      data: {
        ...
        adminDepartment: req.admin.department,
...
router.get('/students/:studentId', verifyFirebaseToken, verifyAdminRole, async (req, res) => {
  try {
    ...
    const student = await CollegeStudent.findOne({
      ...
      department: req.admin.department, // Only show students from admin's department
      isActive: true
    });
...
router.get('/dashboard-stats', verifyFirebaseToken, verifyAdminRole, async (req, res) => {
  try {
    ...
    // Get year-wise distribution for admin's department
    ...
    // Get section-wise distribution for admin's department
    const sectionStats = await CollegeStudent.aggregate([
      { $match: studentsQuery },
      {
        $group: {
          _id: '$section',
