...
import courseDataJson from '../../data/java-tutorial-content.json';
...
if (typeof document !== 'undefined') {
  const existingStyle = document.getElementById('mobile-java-tutorial-styles');
  if (!existingStyle) {
    const style = document.createElement('style');
    style.id = 'mobile-java-tutorial-styles';
...
const JavaTutorial = () => {
  usePageTitle("Java Programming Tutorial - Learn Java Programming");
      ...
      if (user && user.uid) {
        ...
        try {
          ...
          if (response.success && response.data) {
            // Find Java Tutorial course progress
            const javaTutorialCourse = response.data.courseProgress?.find(
              cp => cp.courseId === 'java-programming-tutorial'
            );
            
            if (javaTutorialCourse) {
              // Convert MongoDB format to component format
              const componentProgress = {};
              javaTutorialCourse.chaptersCompleted?.forEach(chapter => {
          ...
          const savedProgress = localStorage.getItem('java-tutorial-progress-demo');
          if (savedProgress) {
            try {
  ...
  const trackDailyActivity = useCallback(async (timeSpent = 5) => {
    ...
    try {
      await progressAPI.updateDailyActivity({
        ...
        lessonsCompleted: 1,
        coursesStudied: ['java-programming-tutorial'],
        topicsStudied: ['Java Programming']
      });
    } catch (error) {
  ...
  const saveProgress = useCallback(async (sectionId, contentIndex, showNotification = false) => {
    ...
    if (user && user.uid) {
      ...
      try {
        ...
        await progressAPI.addCompletedChapter('java-programming-tutorial', {
        ...
        await progressAPI.updateCourseProgress('java-programming-tutorial', {
          courseName: 'Java Programming Tutorial',
        ...
        localStorage.setItem('java-tutorial-progress-demo', JSON.stringify(newProgress));
      }
    } else {
