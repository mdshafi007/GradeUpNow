...
import courseDataJson from '../../data/python-tutorial-content.json';
...
if (typeof document !== 'undefined') {
  const existingStyle = document.getElementById('mobile-python-tutorial-styles');
  if (!existingStyle) {
    const style = document.createElement('style');
    style.id = 'mobile-python-tutorial-styles';
...
const PythonTutorial = () => {
  usePageTitle("Python Programming Tutorial - Learn Python Programming");
      ...
      if (user && user.uid) {
        ...
        try {
          ...
          if (response.success && response.data) {
            // Find Python Tutorial course progress
            const pythonTutorialCourse = response.data.courseProgress?.find(
              cp => cp.courseId === 'python-programming-tutorial'
            );
            
            if (pythonTutorialCourse) {
              // Convert MongoDB format to component format
              const componentProgress = {};
              pythonTutorialCourse.chaptersCompleted?.forEach(chapter => {
          ...
          const savedProgress = localStorage.getItem('python-tutorial-progress-demo');
          if (savedProgress) {
            try {
  ...
  const trackDailyActivity = useCallback(async (timeSpent = 5) => {
    ...
    try {
      await progressAPI.updateDailyActivity({
        ...
        lessonsCompleted: 1,
        coursesStudied: ['python-programming-tutorial'],
        topicsStudied: ['Python Programming']
      });
    } catch (error) {
  ...
  const saveProgress = useCallback(async (sectionId, contentIndex, showNotification = false) => {
    ...
    if (user && user.uid) {
      ...
      try {
        ...
        await progressAPI.addCompletedChapter('python-programming-tutorial', {
        ...
        await progressAPI.updateCourseProgress('python-programming-tutorial', {
          courseName: 'Python Programming Tutorial',
        ...
        localStorage.setItem('python-tutorial-progress-demo', JSON.stringify(newProgress));
      }
    } else {
