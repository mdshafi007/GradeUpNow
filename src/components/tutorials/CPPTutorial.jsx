...
if (typeof document !== 'undefined') {
  const existingStyle = document.getElementById('mobile-cpp-tutorial-styles');
  if (!existingStyle) {
    const style = document.createElement('style');
    style.id = 'mobile-cpp-tutorial-styles';
      ...
      if (user && user.uid) {
        ...
        try {
          ...
          if (response.success && response.data) {
            ...
            const cppTutorialCourse = response.data.courseProgress?.find(
              cp => cp.courseId === 'cpp-programming-tutorial'
            );
            
            if (cppTutorialCourse) {
              // Convert MongoDB format to component format
              const componentProgress = {};
              cppTutorialCourse.chaptersCompleted?.forEach(chapter => {
          ...
          const savedProgress = localStorage.getItem('cpp-tutorial-progress-demo');
          if (savedProgress) {
            try {
  ...
  const trackDailyActivity = useCallback(async (timeSpent = 5) => {
    ...
    try {
      await progressAPI.updateDailyActivity({
        ...
        lessonsCompleted: 1,
        coursesStudied: ['cpp-programming-tutorial'],
  ...
  const saveProgress = useCallback(async (sectionId, contentIndex, showNotification = false) => {
    ...
    if (user && user.uid) {
      ...
      try {
        ...
        await progressAPI.addCompletedChapter('cpp-programming-tutorial', {
        ...
        await progressAPI.updateCourseProgress('cpp-programming-tutorial', {
        ...
        localStorage.setItem('cpp-tutorial-progress-demo', JSON.stringify(newProgress));
      }
    } else {
