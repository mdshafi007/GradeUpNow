...
const Profile = () => {
  ...
  useEffect(() => {
    const loadInitialData = async () => {
      ...
      try {
        // Load available skills and interests from database
        ...
        if (skillsResult.data && skillsResult.data.length > 0) {
          ...
          // Use predefined skills (this is expected for new installations)
          setAvailableSkills([
        ...
        if (interestsResult.data && interestsResult.data.length > 0) {
          ...
          // Use predefined interests (this is expected for new installations)
        ...
        if (profileResult.data && profileResult.data.year) {
          // Profile exists and has academic info
          ...
          // Profile doesn't exist or incomplete - first time user setup
          setProfileData(prev => ({
            ...prev,
            fullName: user?.user_metadata?.full_name || '',
            email: user?.email || ''
        ...
        // Error loading profile data - handled silently
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const profileResult = await upsertUserProfile(user.id, {
      ...
      if (profileResult.error) {
        // Profile save error - handled silently
      ...
      const skillsResult = await updateUserSkills(user.id, profileData.skills)
      if (!skillsResult.success) {
        // Skills save error - handled silently
        // Don't throw here, just log the error
        // Skills may not have been saved properly
      } else {
        // Skills saved successfully
      }
      
      // Save interests to Supabase
      const interestsResult = await updateUserInterests(user.id, profileData.interests)
      if (!interestsResult.success) {
        // Interests save error - handled silently
        // Don't throw here, just log the error
        // Interests may not have been saved properly
      } else {
      ...
      // Error saving profile - handled silently
