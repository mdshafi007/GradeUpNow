...
const TutorialViewer = () => {
  ...
  useEffect(() => {
    const loadUserProgress = async () => {
      ...
      try {
        ...
        const completed = await getCompletedLessons(courseId);
        
        console.log('📋 Raw completed lessons from DB:', completed);
        
        // Convert completed lessons to Set format
        // IMPORTANT: completed has section_id (string), but we need sectionIndex (number)
        const completedSet = new Set();
        
        completed.forEach(lesson => {
          // Find the section index by matching section_id
          const sectionIdx = tutorialData.sections.findIndex(s => s.id === lesson.section_id);
          if (sectionIdx !== -1) {
            const key = `${sectionIdx}-${lesson.lesson_index}`;
            completedSet.add(key);
            console.log(`✅ Added to completedSet: ${key} (section: ${lesson.section_id})`);
          }
        });
        
        console.log('📊 Final completedSet:', Array.from(completedSet));
        setCompletedLessons(completedSet);

        // Resume from last position if available
        if (courseProgress?.current_section_id && courseProgress?.current_lesson_index !== null) {
          const sectionIndex = tutorialData.sections.findIndex(
            s => s.id === courseProgress.current_section_id
          );
          if (sectionIndex !== -1) {
            setCurrentSectionIndex(sectionIndex);
            setCurrentLessonIndex(courseProgress.current_lesson_index);
          }
        }
      } catch (error) {
        console.error('Error loading user progress:', error);
      } finally {
        // Ensure loading shows for at least 800ms for smooth UX
  ...
  const goToNextLesson = async () => {
    ...
    if (user) {
      ...
      // Save to database (silent - no loading state)
      const saved = await markLessonComplete(
        courseId,
        sectionId,
        currentLessonIndex,
        totalLessons,
        tutorialData.sections[nextSectionIndex].id,
        nextLessonIndex
      );
      ...
      if (saved) {
        ...
        // Reload completed lessons from database to keep UI in sync
        try {
          const completed = await getCompletedLessons(courseId);
          const completedSet = new Set();
          ...
          completed.forEach(lesson => {
            const sectionIdx = tutorialData.sections.findIndex(s => s.id === lesson.section_id);
            if (sectionIdx !== -1) {
              completedSet.add(`${sectionIdx}-${lesson.lesson_index}`);
            }
          });
          
          setCompletedLessons(completedSet);
          console.log('✅ Progress reloaded from database:', Array.from(completedSet));
        } catch (error) {
